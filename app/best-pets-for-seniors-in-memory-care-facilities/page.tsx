...
const WHY_TOP_PICK_BULLETS = [
  { label: "Safety Risk", note: "No choking hazards, sharp parts, or tip-over risk — safe for unsupervised use." },
  ...
  { label: "Caregiver Burden", note: "No subscriptions, accounts, or app setup — minimal caregiver effort." },
];
    ...
    <PageShell>
          ...
          <div className="flex items-start gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-base font-medium leading-7 text-slate-700">
                Gentle, familiar companions ranked for safety, caregiver ease, and suitability for seniors with memory challenges.
              </p>
      ...
      {topPick && (
        <section className="py-4 sm:py-5 bg-white">
          <div className="container-shell">
            <div className="overflow-hidden rounded-3xl border-2 border-teal-200 shadow-soft">
              <div className="grid gap-0 lg:grid-cols-[1fr_1.2fr_0.9fr]">
                ...
                <div className="flex flex-col justify-center gap-1.5 p-2.5 sm:p-3">
                  ...
                  <div className="grid grid-cols-3 gap-1 pt-0.5">
                    {topPick.features.map((f) => (
                      <div key={f} className="flex flex-row items-center gap-1.5 rounded-xl bg-teal-100 border border-teal-200 px-2 py-1.5 text-xs font-medium text-brand-900">
                        {featureIcon(f, "h-8 w-8 shrink-0 text-teal-700")}
                        <span>{f}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex flex-col items-center gap-2 border-t-2 border-teal-200 bg-white p-2.5 sm:p-3 lg:border-l-2 lg:border-t-0">
                  <ScoreGauge percent={TOP_PICK_PERCENT} rawScore={TOP_PICK_RAW_SCORE} accentColor="text-teal-700" />
                  ...
                  <div className="w-full space-y-1">
                    {TOP_PICK_CRITERIA.map((c) => (
                      <ScoreBar key={c.label} label={c.label} weight={c.weight} score={c.score} barColor="bg-teal-500" />
                    ))}
                  </div>
                </div>
              </div>
            </div>
            <Link href="/best-pets-for-seniors-in-memory-care-facilities/scoring" className="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-trust-600 underline underline-offset-4 hover:text-trust-800">
      ...
      {topPick && WHY_TOP_PICK_BULLETS.length > 0 && (
        <section className="py-4 sm:py-5 bg-teal-100">
          <div className="container-shell">
            ...
            <ul className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {WHY_TOP_PICK_BULLETS.map((b) => (
                <li key={b.label} className="flex items-start gap-2 rounded-2xl border border-teal-200 bg-white p-3">
      ...
      <section className="pb-4 sm:pb-5 bg-white">
        <div className="container-shell">
          ...
          <div className="mt-4 border-t border-teal-200 pt-3 text-center">
            <Link href="/best-pets-for-seniors-in-memory-care-facilities/scoring" className="inline-flex items-center gap-1 text-sm font-semibold text-trust-600 underline underline-offset-4 hover:text-trust-800">
              Show detailed scoring results →
            </Link>
          </div>
        </div>
