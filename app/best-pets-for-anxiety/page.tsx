...
const ICON_MAP: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  "Size And Portability": IconHeartHands,
...
const WHY_TOP_PICK_BULLETS = [
  { label: "Emotional Comfort Potential", note: "Score of 5 — designed to provide genuine emotional comfort and calm." },
  { label: "Sound Level Control", note: "Score of 5 — mute or volume control available, important for sound-sensitive anxiety." },
  { label: "Safety Risk", note: "No sharp edges, small parts, or sudden movements that could trigger anxiety." },
  { label: "Size And Portability", note: "Compact and lightweight — easy to hold in your lap or carry to different rooms." },
];
...
export default function BestForPage() {
  ...
  return (
    <PageShell>
      ...
      <section className="relative overflow-hidden bg-rose-100 py-8 sm:py-10">
        <div
          ...
          style={{
            backgroundImage: [
              ...
              "  rgba(255,228,230,0.7) 6%,",
              "  rgba(255,228,230,0.2) 13%,",
              "  rgba(255,228,230,0) 20%",
              "),",
              "url('/anxiety-hero.png')",
            ].join(" "),
        ...
        <div className="container-shell relative z-10">
          ...
          <p className="mt-2 max-w-2xl text-base font-medium leading-7 text-slate-700">
            Calming, touch-responsive companions ranked for emotional regulation, sound control, and gentle predictable interaction.
          </p>
      ...
      {topPick && (
        <section className="py-4 sm:py-5 bg-white">
          <div className="container-shell">
            <div className="overflow-hidden rounded-3xl border-2 border-rose-200 shadow-soft">
              <div className="grid gap-0 lg:grid-cols-[1fr_1.2fr_0.9fr]">
                ...
                <div className="flex flex-col justify-center gap-1.5 p-2.5 sm:p-3">
                  ...
                  <div className="grid grid-cols-3 gap-1 pt-0.5">
                    {topPick.features.map((f) => (
                      <div key={f} className="flex flex-row items-center gap-1.5 rounded-xl bg-rose-100 border border-rose-200 px-2 py-1.5 text-xs font-medium text-brand-900">
                        {featureIcon(f, "h-8 w-8 shrink-0 text-rose-700")}
                        <span>{f}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex flex-col items-center gap-2 border-t-2 border-rose-200 bg-white p-2.5 sm:p-3 lg:border-l-2 lg:border-t-0">
                  <ScoreGauge percent={TOP_PICK_PERCENT} rawScore={TOP_PICK_RAW_SCORE} accentColor="text-rose-700" />
                  <p className="text-[11px] text-slate-400 text-center">* % relative to top scorer in this group</p>
                  <div className="w-full space-y-1">
                    {TOP_PICK_CRITERIA.map((c) => (
                      <ScoreBar key={c.label} label={c.label} weight={c.weight} score={c.score} barColor="bg-rose-500" />
                    ))}
                  </div>
                </div>
              </div>
            </div>
            <Link href="/best-pets-for-anxiety/scoring" className="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-trust-600 underline underline-offset-4 hover:text-trust-800">
      ...
      {topPick && WHY_TOP_PICK_BULLETS.length > 0 && (
        <section className="py-4 sm:py-5 bg-rose-100">
          <div className="container-shell">
            ...
            <ul className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {WHY_TOP_PICK_BULLETS.map((b) => (
                <li key={b.label} className="flex items-start gap-2 rounded-2xl border border-rose-200 bg-white p-3">
            ...
            {runners.map((product, i) => (
              <div key={product.slug} className="flex flex-col">
                ...
                <BestForCard product={product} note={RUNNER_NOTES[product.slug]} className="flex-1"
                             scorePercent={SCORE_PERCENT[product.slug]} accentColor="text-rose-700" />
              </div>
            ))}
          </div>
          <div className="mt-4 border-t border-rose-200 pt-3 text-center">
            <Link href="/best-pets-for-anxiety/scoring" className="inline-flex items-center gap-1 text-sm font-semibold text-trust-600 underline underline-offset-4 hover:text-trust-800">
              Show detailed scoring results →
            </Link>
          </div>
        </div>
