...
const WHY_TOP_PICK_BULLETS = [
  { label: "Safety Risk", note: "Score of 5 — no sharp edges, small pieces, or unexpected physical hazards." },
  { label: "Sound Level Control", note: "Score of 5 — mutable or volume-adjustable for different sensory tolerances." },
  { label: "Emotional Comfort Potential", note: "Score of 5 — builds genuine emotional connection without unpredictable behaviour." },
  { label: "Simplicity Of Use", note: "Predictable interaction — no apps, accounts, or complex setup required." },
];
...
export default function BestForPage() {
  ...
  return (
    <PageShell>
      ...
      <section className="relative overflow-hidden bg-violet-100 py-8 sm:py-10">
        <div
          ...
          style={{
            backgroundImage: [
              ...
              "  rgba(237,233,254,0.7) 6%,",
              "  rgba(237,233,254,0.2) 13%,",
              "  rgba(237,233,254,0) 20%",
              "),",
              "url('/autism-hero.png')",
            ].join(" "),
        ...
        <div className="container-shell relative z-10">
          ...
          <h1 className="mt-1 text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">
            Best Pets for Autism
          </h1>
          <p className="mt-2 max-w-2xl text-base font-medium leading-7 text-slate-700">
            Gentle, predictable companions ranked for safety, sound control, and calm sensory interaction — suitable across a wide range of sensory profiles.
          </p>
      ...
      {topPick && (
        <section className="py-4 sm:py-5 bg-white">
          <div className="container-shell">
            <div className="overflow-hidden rounded-3xl border-2 border-violet-200 shadow-soft">
              <div className="grid gap-0 lg:grid-cols-[1fr_1.2fr_0.9fr]">
                ...
                <div className="flex flex-col justify-center gap-1.5 p-2.5 sm:p-3">
                  ...
                  <div className="grid grid-cols-3 gap-1 pt-0.5">
                    {topPick.features.map((f) => (
                      <div key={f} className="flex flex-row items-center gap-1.5 rounded-xl bg-violet-100 border border-violet-200 px-2 py-1.5 text-xs font-medium text-brand-900">
                        {featureIcon(f, "h-8 w-8 shrink-0 text-violet-700")}
                        <span>{f}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex flex-col items-center gap-2 border-t-2 border-violet-200 bg-white p-2.5 sm:p-3 lg:border-l-2 lg:border-t-0">
                  <ScoreGauge percent={TOP_PICK_PERCENT} rawScore={TOP_PICK_RAW_SCORE} accentColor="text-violet-700" />
                  <p className="text-[11px] text-slate-400 text-center">* % relative to top scorer in this group</p>
                  <div className="w-full space-y-1">
                    {TOP_PICK_CRITERIA.map((c) => (
                      <ScoreBar key={c.label} label={c.label} weight={c.weight} score={c.score} barColor="bg-violet-500" />
                    ))}
                  </div>
                </div>
              </div>
            </div>
            <Link href="/best-pets-for-autism/scoring" className="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-trust-600 underline underline-offset-4 hover:text-trust-800">
      ...
      {topPick && WHY_TOP_PICK_BULLETS.length > 0 && (
        <section className="py-4 sm:py-5 bg-violet-100">
          <div className="container-shell">
            ...
            <ul className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {WHY_TOP_PICK_BULLETS.map((b) => (
                <li key={b.label} className="flex items-start gap-2 rounded-2xl border border-violet-200 bg-white p-3">
            ...
            {runners.map((product, i) => (
              <div key={product.slug} className="flex flex-col">
                ...
                <BestForCard product={product} note={RUNNER_NOTES[product.slug]} className="flex-1"
                             scorePercent={SCORE_PERCENT[product.slug]} accentColor="text-violet-700" />
              </div>
            ))}
          </div>
          <div className="mt-4 border-t border-violet-200 pt-3 text-center">
            <Link href="/best-pets-for-autism/scoring" className="inline-flex items-center gap-1 text-sm font-semibold text-trust-600 underline underline-offset-4 hover:text-trust-800">
              Show detailed scoring results →
            </Link>
          </div>
        </div>
