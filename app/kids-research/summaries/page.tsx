...
export const metadata = {
  title: "Research Summaries — Interactive Pets and Children",
  description: "Concise summaries of research articles on AI companions and interactive pets for children: empathy, learning, social-emotional growth, and STEM development."
};

export default function KidsResearchSummariesPage() {
  return <PageShell><JsonLd schema={breadcrumbListSchema([{ name: "Home", path: "/" }, { name: "Kids Research", path: "/kids-research" }, { name: "Article Summaries", path: "/kids-research/summaries" }])} /><section className="pt-14 pb-2 sm:pt-16 sm:pb-3 lg:pt-20 lg:pb-3"><div className="container-shell"><SectionHeading as="h1" eyebrow="Article summaries" title="Summaries: Articles on the Effect of Interactive Pets on Children" text="Brief summaries of each research article. Click any title to read the original source." /></div></section><section className="pt-0 pb-10 sm:pb-12"><div className="container-shell"><ol className="space-y-10 list-decimal pl-6 max-w-4xl">{summaries.map((s,i) => <li key={s.url} className="text-slate-700"><Link href={s.url} target="_blank" rel="noopener noreferrer" className="font-semibold text-trust-700 underline hover:text-trust-900">&ldquo;{s.title}&rdquo;</Link><span className="text-slate-600"> — {s.source}</span>{s.lead && <p className="mt-3 text-slate-700">{s.lead}</p>}<ul className="mt-2 list-disc pl-6 space-y-1 text-slate-700">{s.bullets.map(b => <li key={b}>{b}</li>)}</ul>{s.closing && <p className="mt-3 text-slate-700">{s.closing}</p>}</li>)}</ol><p className="mt-12 text-sm text-slate-600"><Link href="/kids-research" className="underline text-trust-700 hover:text-trust-900">&larr; Back to article list</Link></p></div></section></PageShell>;
}
