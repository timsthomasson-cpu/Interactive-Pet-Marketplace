...
export default function KidsResearchPage() {
  return <PageShell><section className="pt-10 pb-3 sm:pt-12 sm:pb-4 lg:pt-14 lg:pb-5"><div className="container-shell"><SectionHeading eyebrow="Research" title="Research on Interactive Pets and Children" text="A growing body of research suggests that interactive and AI-powered pets and toys can support children's interactive learning, personalized engagement, empathy development, technology fluency, and social-emotional growth." /></div></section><section className="pb-4 sm:pb-5"><div className="container-shell max-w-4xl"><Link href="/kids-research/summaries" className="block group"><h3 className="text-2xl font-bold tracking-tight text-trust-700 group-hover:text-trust-900 underline">Articles on the Effect of Interactive Pets on Children</h3><p className="mt-1 text-sm text-slate-600 group-hover:text-slate-900">(Click to see summaries of articles.)</p></Link></div></section><section className="section-pad pt-2"><div className="container-shell"><ol className="space-y-6 list-decimal pl-6 max-w-4xl">{articles.map((article) => <li key={article.url} className="text-slate-700"><Link href={article.url} target="_blank" rel="noopener noreferrer" className="font-semibold text-trust-700 underline hover:text-trust-900">&ldquo;{article.title}&rdquo;</Link><span className="text-slate-600"> — {article.source}</span></li>)}</ol></div></section></PageShell>;
}
