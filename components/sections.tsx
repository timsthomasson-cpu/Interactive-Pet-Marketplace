...
export function ComparePreview() {
  ...
  return <section className="section-pad bg-white"><div className="container-shell"><SectionHeading eyebrow="Compare faster" title="Not sure which smart pet is right?" text="A table like this helps buyers compare type, comfort level, and price without jumping between tabs." /><div className="mt-10 overflow-hidden rounded-3xl border border-coral-200 bg-white shadow-soft"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-cream-100"><tr>{["Product","Type","Best For","Highlight","Price"].map((h)=><th key={h} className="px-5 py-4 font-semibold text-slate-900">{h}</th>)}</tr></thead><tbody>{top.map((product)=><tr key={product.slug} className="border-t border-coral-200"><td className="px-5 py-4 font-semibold text-slate-900">{product.name}</td><td className="px-5 py-4 text-slate-600">{product.type}</td><td className="px-5 py-4 text-slate-600">{product.bestFor.join(", ")}</td><td className="px-5 py-4 text-slate-600">{product.highlight}</td><td className="px-5 py-4 text-slate-900">{product.price}</td></tr>)}</tbody></table></div></div><Link href="/compare" className="btn-primary mt-8">Compare Top Models</Link></div></section>;
}
